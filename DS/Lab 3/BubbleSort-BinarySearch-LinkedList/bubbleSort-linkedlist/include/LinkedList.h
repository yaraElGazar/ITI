...
class LinkedList
{
    ...
    public:
        ...
        void BubbleSort()
        {
            ...
            {
                // Changing the flag as if it is the last iteration
                sorted = 1;
                for (int index = 0; index < Size - 1 - counter; index++)
                {
                    if (GetDataByIndex(index) > GetDataByIndex(index + 1))
                    {
                        // Swap

                        // Get the 2 nodes
                        Node* NodeA = GetNodeByData (GetDataByIndex(index));
                        Node* NodeB = GetNodeByData (GetDataByIndex(index + 1));
                        // Swapping Data between the two nodes
                        int temp = NodeA -> Data;
                        NodeA -> Data = NodeB -> Data;
                        NodeB -> Data = temp;

                        // Setting the flag to continue the iterations
                        sorted = 0;
                    }
                }
            }
        ...
        // Takes data and returns index if found and -1 if does not exist
        int BinarySearch(int data)
        {
            // The linkedList must be sorted
            BubbleSort();
            ...
            while (minIndex <= maxIndex)
            {
                ...
                // If data is found in the middle
