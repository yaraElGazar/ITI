...
describe("test capitalizeText function", () => {
  ...
  it("will return a string", () => {
    chai.assert.isString(capitalizeText("hamada"));
    chai.expect(capitalizeText("hamada")).to.be.an("string");
  ...
  it("will return a string after capitalizing it", () => {
    chai.assert.equal(capitalizeText("hamada"), "HAMADA");
    chai.expect(capitalizeText("hamada")).to.equal("HAMADA");
...
describe("test createArray function", () => {
  ...
  it("return value of type array", () => {
    chai.assert.isArray(createArray(3));
    chai.expect(createArray(3)).to.be.an("array");
    createArray(3).should.to.be.an("array");
  });
  //? test if we pass 3 it will return array of length 3 and test it's include 1
  it("return value of type array", () => {
    chai.assert.isArray(createArray(3));
    chai.assert.lengthOf(createArray(3), 3);
    chai.assert.include(createArray(3), 1);
